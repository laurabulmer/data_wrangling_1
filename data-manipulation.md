...
can also use rename if you want to rename a few without getting rid of
...
basically there are a bunch of helper functions that do things that you
can also do with select.

## ‘filter’

while select operates on columns, filter operates on rows. filter needs
...
use the double equal sign when posing the question , rather than telling
it to set things equal to 20
...
adding the exclamation point outside of the parenthesis does the
opposite, gives you everything where its not equal to 20. we also added
a second way to do this below
